...
ORG_ROLES=$(ibmcloud account org-role-set -h | \
  awk ' \
...
print_help()
{
  ...
  echo "                Must be one of:"
  printf '%b' "`(ibmcloud regions | awk 'NR>3 {print "\t\t\t",$1}' -)`\n"
...
do
  ...
  ibmcloud account org-user-add $USER "$ORG" 
  for ROLE in $ORG_ROLES
  do
    ibmcloud account org-role-set $USER "$ORG"  $ROLE
  done
